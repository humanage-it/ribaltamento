...
public class OfferingsNoRegistrationsCommand
extends SabaWebCommand
{
	public OfferingsNoRegistrationsCommand()
	{
	...
	private List<Offering> doSearch()
	{
		...
		try
		{
			...
			ps = connection.prepareStatement(OfferingsNoRegistrationsCommand.kSearchSelect);
			rs = ps.executeQuery();
		...
		{
			System.out.println("OfferingsNoRegistrationsCommand doSearch Exception = " + ex.getMessage());
			Debug.trace("OfferingsNoRegistrationsCommand doSearch Exception = " + ex.getMessage());
		}
		finally
		{
			try {
				...
				Debug.trace("OfferingsNoRegistrationsCommand doSearch Exception finally = " + ex.getMessage());
			}
		}
	...
	private static final String kSearchSelect =
		"select Id, Title, OfferingNo, Location, StartDate from UGF_FONSAI_OFFERINGS_NO_REG order by StartDate desc, Location";
}
